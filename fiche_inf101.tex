...
Une coupe $(S,\overline{S})$ est une bipartition de $X$ avec :\begin{enumerate}
\item $S \cap \overline{S} = \varnothing$
\item $S \cup \overline{S} = X$
\item $s\in S$ et $p\in \overline{S}$
\end{enumerate}

On définit $c(S,\overline{S}) = \sum_{(x,y)\in S\times\overline{S}}{c(x,y)}$.

Soit $f$ un flot de $s$ à $p$. Soit $(S,\overline{S})$ une coupe dans $G$. Alors :
\[ \displaystyle v(f) = \sum_{(x,y)\in S\times\overline{S}}{f(x,y)} - \sum_{(z,t)\in \overline{S}\times S}{f(z,t)} \]

\textbf{Corollaire 1 :} $\forall\,\text{flot}\,f, \,\forall\,\text{coupe}\,(S,\overline{S})\\v(f) \leqslant c(S,\overline{S})$.

\textbf{Corollaire 2 :} Soit $v_{\text{max}}$ la valeur maximale d'un flot et soit $c_{\text{min}}$ la capacité minimum d'une coupe. Alors $v_{\text{max}} \leqslant c_{\text{min}}$. De plus, s'il existe $f$ et $(S,\overline{S})$ avec $v(f) = c(S,\overline{S})$ alors $f$ est un flot maximum et $(S,\overline{S})$ une coupe de capacité maximum.
